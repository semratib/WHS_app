...
# total population
total_pop <- cod19 %>% 
  filter(sex=="Both sexes" & DIM_AGEGROUP_CODE == 101 & DIM_GHECAUSE_CODE==0) %>%
  group_by(DIM_YEAR_CODE) %>% 
  summarise(pop = sum(ATTR_POPULATION_NUMERIC)) %>% 
  pull(pop) %>% 
  format(big.mark = ",", scientific = FALSE)
  
life_exp <- ledata %>% filter(iso3 == "World" & sex=="Both sexes") %>% pull(value)
  
# total number of deaths
total_deaths <- cod19 %>%
  filter(sex=="Both sexes" & DIM_AGEGROUP_CODE == 101 & FLAG_LEVEL == 0) %>%
  group_by(DIM_YEAR_CODE) %>% 
...
lev1 <- cod19 %>%
  filter(sex=="Both sexes" & DIM_AGEGROUP_CODE == 101 & FLAG_LEVEL == 1) %>%
...
lev2 <- cod19 %>%
  filter(sex=="Both sexes" & DIM_AGEGROUP_CODE == 101 & FLAG_LEVEL == 2) %>%
  group_by(DIM_YEAR_CODE, DIM_GHECAUSE_TITLE) %>% 
  summarise(dths = sum(VAL_DEATHS_COUNT_NUMERIC)) %>% 
  ungroup() %>%
  mutate(cf = round(100 * dths / total_deaths),
...
lev2comp <- cod00 %>%
  filter(sex=="Both sexes" & FLAG_LEVEL == 2) %>%
  group_by(DIM_YEAR_CODE, DIM_GHECAUSE_TITLE) %>% 
  summarise(dths = sum(VAL_DEATHS_COUNT_NUMERIC)) %>% 
  ungroup() %>%
  mutate(cf = round(100 * dths / total_deaths),
...
data_treemap <- cod19 %>%
  filter(sex=="Both sexes" & DIM_AGEGROUP_CODE==101 & FLAG_TREEMAP == 1) %>% 
...
total_deaths <- cod19 %>% 
  filter(FLAG_LEVEL == 0 & sex=="Both sexes" & DIM_AGEGROUP_CODE==101) %>%
  group_by(DIM_YEAR_CODE, DIM_GHECAUSE_TITLE) %>% 
  summarise(VAL_DEATHS_COUNT_NUMERIC = sum(VAL_DEATHS_COUNT_NUMERIC)) %>% 
  ungroup() %>% 
...
data_treemap <- cod00 %>%
  filter(sex=="Both sexes" & FLAG_TREEMAP == 1) %>% 
  group_by(DIM_YEAR_CODE, DIM_GHECAUSE_TITLE, FLAG_CAUSEGROUP) %>% 
  summarise(VAL_DEATHS_COUNT_NUMERIC = sum(VAL_DEATHS_COUNT_NUMERIC)) %>% 
  ungroup()
...
data_treemap <- data_treemap %>%
  ...
  # get total death count for title
total_deaths <- cod00 %>% 
  filter(FLAG_LEVEL == 0 & sex=="Both sexes") %>%
  group_by(DIM_YEAR_CODE, DIM_GHECAUSE_TITLE) %>% 
  summarise(VAL_DEATHS_COUNT_NUMERIC = sum(VAL_DEATHS_COUNT_NUMERIC)) %>% 
  ungroup() %>% 
  pull(VAL_DEATHS_COUNT_NUMERIC) %>% round()
...
# donut_filtered <- prep_donut_data()
year_pie = 2018
country_iso_ <- tolower(country_iso)
donut_data <- sdg_data %>%
    filter(billion == "uhc") %>%
    filter(aggregate_id == country_iso_) %>%
...
make_arrow(cod19 %>% 
             filter(sex=="Both sexes" & DIM_AGEGROUP_CODE==101) %>% 
             group_by(FLAG_CAUSEGROUP, DIM_YEAR_CODE, DIM_GHECAUSE_TITLE, FLAG_LEVEL) %>% 
             summarise(VAL_DEATHS_COUNT_NUMERIC = sum(VAL_DEATHS_COUNT_NUMERIC)) %>% 
             ungroup() %>% 
             mutate(DIM_COUNTRY_CODE = "Global"), 
