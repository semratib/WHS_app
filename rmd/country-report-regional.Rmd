...
# total population
total_pop <- cod19 %>% 
  filter(sex=="Both sexes" & DIM_AGEGROUP_CODE == "TOTAL" & region == country_iso & FLAG_LEVEL == 0) %>%
...
testiso3 <- case_when(
  country_iso=="AFR" ~ "1_Afr",
  country_iso=="AMR" ~ "2_Amr",
  country_iso=="EMR" ~ "5_Emr",
  country_iso=="EUR" ~ "4_Eur",
  country_iso=="SEAR" ~ "3_Sear",
  country_iso=="WPR" ~ "6_Wpr",
)

# life_exp <- ledata %>% filter(GHOcode=="WHOSIS_000001" & sex=="Both sexes" & year==2021 & iso3 == testiso3) %>% pull(value)
life_exp <- data %>% filter(GHOcode=="WHOSIS_000001" & sex=="Both sexes" & year==2021 & country == country_) %>% pull(value)

# life_exp <- ledata %>% filter(iso3 == testiso3 & sex=="Both sexes") %>% pull(value)
...
lev1 <- cod19 %>%
  filter(sex=="Both sexes" & DIM_AGEGROUP_CODE == "TOTAL" & region == country_iso & FLAG_LEVEL == 1) %>%
...
lev2 <- cod19 %>%
  filter(sex=="Both sexes" & DIM_AGEGROUP_CODE == "TOTAL" & region == country_iso & FLAG_LEVEL == 2) %>%
  group_by(DIM_YEAR_CODE, DIM_GHECAUSE_TITLE) %>% 
  summarise(dths = sum(VAL_DEATHS_COUNT_NUMERIC)) %>% 
  ungroup() %>%
  mutate(cf = round(100 * dths / total_deaths),
...
death_rate <- cod19 %>%
  filter(region == country_iso & sex=="Both sexes" & DIM_AGEGROUP_CODE == "TOTAL" & FLAG_LEVEL == 0) %>%
  group_by(DIM_YEAR_CODE) %>% 
...
In 2021, `r country` had a life expectancy at birth of `r round(life_exp, 1)` years.\\
\\
`r country` had `r format(round(total_deaths), big.mark=",", scientific=F)` total deaths in 2021 and the mortality rate was `r death_rate` deaths per 100,000 population.\\
\\
...
top <- top10 %>% 
  filter(region==country_ & sex == "Both sexes") %>% 
  ungroup() %>% 
...
d <- cod19 %>%
  filter(region2 == country_ & FLAG_LEVEL==1 & sex=="Both sexes" & DIM_AGEGROUP_CODE=="TOTAL") %>%
  group_by(region2, DIM_GHECAUSE_TITLE, DIM_YEAR_CODE) %>%
  summarise(VAL_DEATHS_COUNT_NUMERIC = sum(VAL_DEATHS_COUNT_NUMERIC)) %>%
  ungroup() %>% 
  rename(country = region2) %>% 
...
b <- cod19 %>% 
  filter(region2 == country_ & FLAG_LEVEL==2 & sex=="Both sexes" & DIM_AGEGROUP_CODE=="TOTAL") %>%
  group_by(region2, DIM_GHECAUSE_TITLE, DIM_YEAR_CODE, FLAG_CAUSEGROUP) %>%
  summarise(VAL_DEATHS_COUNT_NUMERIC = sum(VAL_DEATHS_COUNT_NUMERIC)) %>%
  ungroup() %>%
...
data_treemap <- cod19 %>%
    filter(region == country_iso & sex=="Both sexes" & DIM_AGEGROUP_CODE=="TOTAL" & FLAG_TREEMAP == 1)
...
total_deaths <- cod19 %>% 
  filter(region == country_iso & FLAG_LEVEL == 0 & sex=="Both sexes" & DIM_AGEGROUP_CODE=="TOTAL") %>%
  group_by(DIM_YEAR_CODE) %>% 
  summarise(dths = sum(VAL_DEATHS_COUNT_NUMERIC)) %>% 
  pull(dths) %>% round()
...
\newpage
\normalsize Causes of death within `r who_region_name`
```{r heatmap, fig.align='center', fig.height=h1}
metric = "deaths" #fig.height=9

data_heatmap <- cod19 %>%
    filter(DIM_COUNTRY_CODE %in% who_region_countries &
             FLAG_LEVEL == 2 &
             sex=="Both sexes" & DIM_AGEGROUP_CODE=="TOTAL") %>%
    group_by(DIM_COUNTRY_CODE) %>%
    mutate(prop_deaths = VAL_DEATHS_COUNT_NUMERIC / sum(VAL_DEATHS_COUNT_NUMERIC),
           rank_deaths = rank(-prop_deaths, ties.method = "first")) %>%
    ungroup() %>%
    rename(rank = paste0("rank_", metric)) %>%
    select(DIM_COUNTRY_CODE, country, DIM_GHECAUSE_TITLE, rank)

data_heatmap_region <- cod19 %>%
  filter(region2==country_ & 
           FLAG_LEVEL == 2 &
           sex=="Both sexes" & DIM_AGEGROUP_CODE=="TOTAL") %>%
  group_by(region, region2, DIM_GHECAUSE_TITLE) %>%
  summarise(VAL_DEATHS_COUNT_NUMERIC = sum(VAL_DEATHS_COUNT_NUMERIC)) %>% 
  ungroup() %>% 
  mutate(prop_deaths = VAL_DEATHS_COUNT_NUMERIC / sum(VAL_DEATHS_COUNT_NUMERIC),
         rank_deaths = rank(-prop_deaths, ties.method = "first")) %>%
  rename(rank = paste0("rank_", metric)) %>%
  select(DIM_COUNTRY_CODE = region, country = region2, DIM_GHECAUSE_TITLE, rank)

data_heatmap <- rbind(data_heatmap_region, data_heatmap)
...
country_order <- c(country_, setdiff(unique(data_heatmap$country), country_))
data_heatmap$country <- factor(data_heatmap$country, levels = rev(country_order))
  
# rank causes by country-specific death ranking
cause_order <- data_heatmap %>% 
...
data_heatmap$country = str_wrap(data_heatmap$country, width = 40)
  
  # make plot
  
h_iso <- ggplot(data_heatmap %>% filter(DIM_COUNTRY_CODE==country_iso), 
       aes(x=DIM_GHECAUSE_TITLE, y=country, fill=rank, label=rank, family="calibri")) +
    geom_tile(color = "black") +
    geom_text(aes(label = rank)) +
    theme_classic() +
    theme(legend.position = "none",
          axis.text.x = element_text(angle = -45, hjust=1, family="calibri", size = 8),
          axis.text.y = element_text(face = country_face, family="calibri", size = 8, hjust = 15),
...
h_reg <- ggplot(data_heatmap %>% filter(DIM_COUNTRY_CODE!=country_iso), 
       aes(x=DIM_GHECAUSE_TITLE, y=country, fill=rank, label=rank, family="calibri")) +
    geom_tile(color = "black") +
    theme_classic() +
    theme(legend.position = "none",
          axis.text.x = element_blank(),
          axis.text.y = element_text(face = country_face, family="calibri", size = 8, hjust = 0),
          axis.ticks = element_blank(),
          axis.line = element_blank(), 
          axis.text = element_text(color = "black"),
...
if(who_region=="SEAR"){
  ...
  patchwork::wrap_plots(h_iso, h_reg, ncol = 1, heights = c(2, 12))

}else{
  
  patchwork::wrap_plots(h_iso, h_reg, ncol = 1, heights = c(1, 20))

}
```

\newpage
Causes of death by sex, 2021
```{r codbysex, fig.align='center', fig.height=4}

d2 <- cod19 %>% 
  filter(region2 == country_ & FLAG_LEVEL==1 & sex %in% c("Male", "Female") & DIM_AGEGROUP_CODE=="TOTAL") %>%
...
```
\newpage
\normalsize `r who_region_name`: Triple Billion indicator progress – likelihood of achieving by 2030
...
indicator_values <- filtered_indicator_values %>% 
    ...
    filter(aggregate_id %in% who_region_countries | aggregate_id==country_iso) %>%
...
bh_iso <- ggplot(indicator_values %>% filter(aggregate_id==country_iso), 
    ...
    theme(legend.position = "top",
          ...
          # legend.text = element_text(margin = margin(r = 9, t = 0, l = 3, b = 0, unit = "pt")),
...
if(who_region_name=="African Region"){
