...
# total population
total_pop <- cod19 %>% 
  filter(sex=="Both sexes" & DIM_AGEGROUP_CODE == 101 & region == country_iso & FLAG_LEVEL == 0) %>%
...
lev1 <- cod19 %>%
  filter(sex=="Both sexes" & DIM_AGEGROUP_CODE == 101 & region == country_iso & FLAG_LEVEL == 1) %>%
...
lev2 <- cod19 %>%
  filter(sex=="Both sexes" & DIM_AGEGROUP_CODE == 101 & region == country_iso & FLAG_LEVEL == 2) %>%
  group_by(DIM_YEAR_CODE, DIM_GHECAUSE_TITLE) %>% 
  summarise(dths = sum(VAL_DEATHS_COUNT_NUMERIC)) %>% 
  ungroup() %>%
  mutate(cf = round(100 * dths / total_deaths),
...
lev2comp <- cod00 %>%
  filter(sex=="Both sexes" & region == country_iso & FLAG_LEVEL == 2) %>%
  group_by(DIM_YEAR_CODE, DIM_GHECAUSE_TITLE) %>% 
  summarise(dths = sum(VAL_DEATHS_COUNT_NUMERIC)) %>% 
  ungroup() %>%
  mutate(cf = round(100 * dths / total_deaths),
...
death_rate <- cod19 %>%
  filter(region == country_iso & sex=="Both sexes" & DIM_AGEGROUP_CODE == 101 & FLAG_LEVEL == 0) %>%
  group_by(DIM_YEAR_CODE) %>% 
...
data_treemap <- cod19 %>%
  filter(sex=="Both sexes" & DIM_AGEGROUP_CODE==101 & FLAG_TREEMAP == 1 & region == country_iso) %>% 
...
total_deaths <- cod19 %>% 
  filter(FLAG_LEVEL == 0 & sex=="Both sexes" & DIM_AGEGROUP_CODE==101 & region == country_iso) %>%
  group_by(DIM_YEAR_CODE, DIM_GHECAUSE_TITLE) %>% 
  summarise(VAL_DEATHS_COUNT_NUMERIC = sum(VAL_DEATHS_COUNT_NUMERIC)) %>% 
  ungroup() %>% 
...
data_treemap <- cod00 %>%
  filter(sex=="Both sexes" & FLAG_TREEMAP == 1 & region == country_iso) %>% 
  group_by(DIM_YEAR_CODE, DIM_GHECAUSE_TITLE, FLAG_CAUSEGROUP) %>% 
  summarise(VAL_DEATHS_COUNT_NUMERIC = sum(VAL_DEATHS_COUNT_NUMERIC)) %>% 
  ungroup()
...
data_treemap <- data_treemap %>%
  ...
  # get total death count for title
total_deaths <- cod00 %>% 
  filter(FLAG_LEVEL == 0 & sex=="Both sexes" & region == country_iso) %>%
  group_by(DIM_YEAR_CODE, DIM_GHECAUSE_TITLE) %>% 
  summarise(VAL_DEATHS_COUNT_NUMERIC = sum(VAL_DEATHS_COUNT_NUMERIC)) %>% 
  ungroup() %>% 
  pull(VAL_DEATHS_COUNT_NUMERIC) %>% round()
...
make_arrow(cod19 %>% 
             filter(sex=="Both sexes" & DIM_AGEGROUP_CODE==101 & region == country_iso) %>% 
             group_by(FLAG_CAUSEGROUP, DIM_YEAR_CODE, DIM_GHECAUSE_TITLE, FLAG_LEVEL) %>% 
             summarise(VAL_DEATHS_COUNT_NUMERIC = sum(VAL_DEATHS_COUNT_NUMERIC)) %>% 
             ungroup() %>% 
             mutate(DIM_COUNTRY_CODE = country_iso), 
           cod00 %>% filter(sex=="Both sexes" & region == country_iso) %>% 
             group_by(FLAG_CAUSEGROUP, DIM_YEAR_CODE, DIM_GHECAUSE_TITLE, FLAG_LEVEL) %>% 
             summarise(VAL_DEATHS_COUNT_NUMERIC = sum(VAL_DEATHS_COUNT_NUMERIC)) %>% 
             ungroup()%>% 
             mutate(DIM_COUNTRY_CODE = country_iso), 
...
data_barchart <- cod19 %>% 
  filter(sex=="Both sexes" & DIM_AGEGROUP_CODE!=101 & FLAG_LEVEL == 1 & region == country_iso) %>% 
  group_by(DIM_AGEGROUP_CODE, DIM_YEAR_CODE, DIM_GHECAUSE_TITLE) %>% 
  summarise(VAL_DEATHS_COUNT_NUMERIC = sum(VAL_DEATHS_COUNT_NUMERIC)) %>% 
  ungroup() %>% 
...
knitr::include_graphics(paste0("SDG381_region_tables/rmnch_infec_", country_iso, ".pdf"))
```
...
knitr::include_graphics(paste0("SDG381_region_tables/ncd_serv_", country_iso, ".pdf"))
```
...
knitr::include_graphics(paste0("SDG381_region_tables/sci_uhc_", country_iso, ".pdf"))
```
