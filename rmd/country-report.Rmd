...
life_exp <- data %>% filter(GHOcode=="WHOSIS_000001" & sex=="Both sexes" & year==2021 & iso3 == country_iso) %>% pull(value)
...
\\
`r country` had `r format(round(total_deaths), big.mark=",", scientific=F)` total deaths in 2021 and the mortality rate was `r death_rate` deaths per 100,000 population. In `r who_region_name`, this mortality rate is lower than `r nrow(mort_region) - death_rank` countries and higher than `r death_rank - 1` countries.\\
\\
...
h_iso <- ggplot(data_heatmap %>% filter(DIM_COUNTRY_CODE==country_iso), 
       aes(x=DIM_GHECAUSE_TITLE, y=country_name, fill=rank, label=rank, family="calibri")) +
...
d2 <- cod19 %>% 
  ...
  select(country, sex, DIM_GHECAUSE_TITLE, VAL_DEATHS_COUNT_NUMERIC, ATTR_POPULATION_NUMERIC) %>% 
  rbind(cod19 %>% 
  ...
  # mutate(label_ = ifelse(country == who_region_name & sex=="Female" & DIM_GHECAUSE_TITLE=="Communicable, maternal, perinatal and nutritional conditions",
  #                        who_region_name, NA))
...
# make plot
ggplot() + 
  geom_col(data = d2 %>% filter(country== who_region_name),
           aes(x= str_wrap(DIM_GHECAUSE_TITLE, 15), y = deaths_100, fill = DIM_GHECAUSE_TITLE), width = 0.8, alpha = 0.3, show.legend = F)+
  geom_col(data = d2 %>% filter(country== country_),
           aes(x= str_wrap(DIM_GHECAUSE_TITLE, 15), y = deaths_100, fill = DIM_GHECAUSE_TITLE), width = 0.4, show.legend = F, color = "black") +
  # geom_text_repel(data = d2 %>% filter(country== who_region_name), 
  #           aes(y = deaths_100, x = DIM_GHECAUSE_TITLE, label = label_), size = 2) +
  facet_share(~sex, dir = "h", 
                scales = 'free') + 
  coord_flip(clip = 'off') +
    # annotate("text", 
    #        x = 4, 
    #        y = d2 %>% filter(country== who_region_name & sex=="Male" & DIM_GHECAUSE_TITLE=="Communicable, maternal, perinatal and nutritional conditions") %>% pull(deaths_100), 
    #       label = who_region_name , color="black",
...
ggplot() + 
  ...
  coord_flip(clip = 'off') +
      # annotate("text", 
      #      x = 4, 
      #      y = d3 %>% filter(country== who_region_name & sex=="Male" & DIM_GHECAUSE_TITLE=="Communicable, maternal, perinatal and nutritional conditions") %>% pull(daly_100), 
      #     label = who_region_name , color="black",
      #     size=2, fontface="bold") +
  scale_fill_manual(values = colors)+
  scale_x_discrete(limits = rev)+
...
# wrap_plots(bh_iso, bh_reg, ncol = 1, heights = c(t1, t2))
```
\newpage
